...
/**
 * <P>Preferences的一个工具类，从其他地方拷贝过来的，原文如下：</P>
 * <P>GitHub:<a href="https://github.com/Pixplicity/EasyPreferences">https://github.com/Pixplicity/EasyPreferences</a></P>
 * @author Pixplicity
 */
...
public final class EasyPreferences {
    ...
    private static SharedPreferences mEasyPreferences;

    /**
     * Initialize the EasyPreferences helper class to keep a reference to the SharedPreference for this
     ...
     */
    @Deprecated
    public static void initEasyPreferences(Context context) {
        new Builder().setContext(context).build();
    }

    private static void initEasyPreferences(Context context, String EasyPreferencesName, int mode) {
        mEasyPreferences = context.getSharedPreferences(EasyPreferencesName, mode);
    }
    ...
    public static SharedPreferences getPreferences() {
        if (mEasyPreferences != null) {
            return mEasyPreferences;
        }
        throw new RuntimeException(
                "EasyPreferences class not correctly instantiated. Please call Builder.setContext().build() in the Application class onCreate.");
    }
    ...
    public static Set<String> getStringSet(final String key, final Set<String> defValue) {
        SharedPreferences EasyPreferences = getPreferences();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            return EasyPreferences.getStringSet(key, defValue);
        } else {
    ...
    public static Set<String> getOrderedStringSet(String key, final Set<String> defValue) {
        SharedPreferences EasyPreferences = getPreferences();
        if (EasyPreferences.contains(key + LENGTH)) {
            LinkedHashSet<String> set = new LinkedHashSet<>();
            int stringSetLength = EasyPreferences.getInt(key + LENGTH, -1);
            if (stringSetLength >= 0) {
                for (int i = 0; i < stringSetLength; i++) {
                    set.add(EasyPreferences.getString(key + "[" + i + "]", null));
                }
            }
            return set;
        }
    ...
    public static void putOrderedStringSet(String key, Set<String> value) {
        ...
        if (mEasyPreferences.contains(key + LENGTH)) {
            // First read what the value was
            stringSetLength = mEasyPreferences.getInt(key + LENGTH, -1);
        }
    ...
    public static void remove(final String key) {
        SharedPreferences EasyPreferences = getPreferences();
        final Editor editor = EasyPreferences.edit();
        if (EasyPreferences.contains(key + LENGTH)) {
            // Workaround for pre-HC's lack of StringSets
            int stringSetLength = EasyPreferences.getInt(key + LENGTH, -1);
            if (stringSetLength >= 0) {
    ...
    /**
     * Builder class for the EasyEasyPreferences instance. You only have to call this once in the Application
     * onCreate. And in the rest of the code base you can call EasyPreferences.method name.
     */
    public final static class Builder {
         ...
         *
         * @param EasyPreferencesName the filename used for the SharedPreference
         * @return the {@link EasyPreferences.Builder} object.
         */
        public Builder setEasyPreferencesName(final String EasyPreferencesName) {
            mKey = EasyPreferencesName;
            return this;
        }
        ...
        public void build() {
            if (mContext == null) {
                throw new RuntimeException("Context not set, please set context before building the EasyPreferences instance.");
            }
            ...
            EasyPreferences.initEasyPreferences(mContext, mKey, mMode);
        }
    }
}